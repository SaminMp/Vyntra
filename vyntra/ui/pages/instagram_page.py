"""
Dedicated Instagram Platform Page for Vyntra.
Supports Reel and Post URL extraction, preview playback, and MP4/MP3 downloading.
"""
...
class InstagramPage(BasePlatformPage):
    """
    Dedicated view for Instagram Reels and Posts.
    Provides direct URL-based media extraction, preview streaming, and downloading.
    """

    def __init__(self, master, app, **kwargs):
        super().__init__(master, app, platform_service=instagram_platform, **kwargs)

        self._current_item: Optional[MediaItem] = None
        self._is_loading = False

        self.grid_columnconfigure(0, weight=1)
        self.grid_rowconfigure(2, weight=1)  # Preview card area expands
    ...
    def _build_header(self):
        ...
        meta = PLATFORM_METADATA.get("instagram")
        icon = meta.icon if meta else "◎"
        subtitle_text = meta.subtitle if meta else "Search, watch and download supported reels and posts"

        title = ctk.CTkLabel(
            header_frame,
            text=f"{icon}  Instagram",
    ...
    def _build_url_section(self):
        ...
        self.url_entry = ctk.CTkEntry(
            input_card,
            placeholder_text="https://www.instagram.com/reel/... or /p/...",
            font=Theme.FONT_BODY,
            height=40,
            border_width=1,
    ...
    def _build_preview_card_section(self):
        ...
        self.placeholder_label = ctk.CTkLabel(
            ...
            text="Paste an Instagram link above and click 'Fetch Media' to preview or download.",
            font=Theme.FONT_BODY,
        ...
        # Title & Author
        self.title_label = ctk.CTkLabel(self.card_content, text="Media Title", font=Theme.FONT_HEADER, text_color=Theme.TEXT_PRIMARY, anchor="w")
        ...
        self.meta_label = ctk.CTkLabel(self.card_content, text="Reel • MP4", font=Theme.FONT_CAPTION, text_color=Theme.TEXT_MUTED, anchor="w")
    ...
    def _handle_fetch(self):
        ...
        if not url:
            self.app.status_banner.show_warning("Please paste an Instagram URL.")
            return

        if not self.platform_service.can_handle_url(url):
            self.app.status_banner.show_warning("Invalid Instagram URL. Expected https://www.instagram.com/reel/... or /p/...")
            return

        gen = self.next_generation()
        self._set_loading(True)
        if self.placeholder_label.winfo_exists():
            self.placeholder_label.configure(text="Extracting Instagram media metadata...")
    ...
    def _set_loading(self, loading: bool):
        ...
        if self.status_msg.winfo_exists():
            self.status_msg.configure(text="Connecting to Instagram..." if loading else "Ready")
    ...
    def _display_item(self, item: MediaItem, generation: int):
        ...
        if self.meta_label.winfo_exists():
            self.meta_label.configure(text=f"{dur_text}  •  Instagram Original")
    ...
    def _display_error(self, err: Exception, generation: int):
        ...
        self._set_loading(False)
        if self.placeholder_label.winfo_exists():
            self.placeholder_label.configure(text=f"Failed to load Instagram media:\n{str(err)}")
            self.placeholder_label.place(relx=0.5, rely=0.5, anchor="center")
        if self.card_content.winfo_exists():
            self.card_content.pack_forget()
        self.app.status_banner.show_error(f"Instagram extraction error: {str(err)}")
